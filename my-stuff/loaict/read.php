...
while ($row = $re->fetch_assoc()) {
	if (isset($_GET['id'])) {
		if ($row['ma_loai'] == $_GET['id']) {
			echo '<tr><td colspan="5"><form action="'.home_url().'/my-stuff/loaict/update.php" method="POST">';
			echo '<table>';
		...
		echo '<td><a class="btn btn-info" href="'.home_url('/loaict/').'?id=' . $row["ma_loai"] . '">Sửa</a></td>';
