...
echo "<tbody>";
echo "<thead>";
echo "<th>Tên loại công trình</th><th>Update</th>";
echo "</thead>";
while ($row = $re->fetch_assoc()) {
	if (isset($_GET['id'])) {
		if ($row['ma_loai'] == $_GET['id']) {
			...
			echo '<td><input type="hidden" name="ma_loai" value="' . $row['ma_loai'] . '"><input type="submit" value="Save"></td>';
			echo '</tr>';
			echo '</form>';
			echo '</td></td>';
		}
		...
		echo '<td><a class="btn btn-info" href="'.$pagename.'?id=' . $row["ma_loai"] . '">Update</a></td>';
