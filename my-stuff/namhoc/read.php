...
echo "<tbody>";
echo "<thead>";
echo "<th>Năm học</th><th>Update</th>";
echo "</thead>";
while ($row = $re->fetch_assoc()) {
	if (isset($_GET['id'])) {
		if ($row['ma_nh'] == $_GET['id']) {
			...
			echo '<td><input type="hidden" name="ma_nh" value="' . $row['ma_nh'] . '"><input type="submit" value="Save"></td>';
			echo '</tr>';
			echo '</form>';
			echo '</td></td>';
		}
		...
		echo '<td><a class="btn btn-info" href="'.$pagename.'?id=' . $row["ma_nh"] . '">Update</a></td>';
