...
$sql4= "SELECT namhoc FROM `NamHoc` WHERE ma_nh = $a ";
	...
	$sql2 = "SELECT dtnc.`ma_dtnckh`,cb.ma_cb, max(cbcd.id),cd.ten_cd, dtnc.ten_dtnckh, dtnc.nam_kethuc,dtnc.sluong_thamgia, dtcb.ten_loaivt, cd.dinhmuc, cdt.ten_cdt,cdt.giochuan,khoa.ten_khoa FROM `DeTai_NCKH` dtnc INNER JOIN `DeTai_CanBo` dtcb on dtnc.ma_dtnckh=dtcb.ma_dtnckh INNER JOIN `Canbo` cb on cb.ma_cb=dtcb.macb INNER JOIN `CanBo_ChucDanh` cbcd on cbcd.ma_cb=cb.ma_cb INNER JOIN `NamHoc` nh on dtnc.ma_nh = nh.ma_nh INNER JOIN `ChucDanh` cd on cd.ma_cd=cbcd.ma_cd INNER JOIN `CapDeTai` cdt ON cdt.ma_cdt=dtnc.ma_cdt INNER JOIN `Khoa_PB` khoa on khoa.ma_khoa = cb.ma_khoa WHERE nh.ma_nh = $a and cb.user_id = $user_id and dtnc.trangthai = 1  GROUP by dtnc.`ma_dtnckh`,cb.ma_cb";
    
	$re = $conn->query($sql2);

	error_log('sql = ' . $sql2);

    $sql1 = "SELECT gt.ma_gt,cb.ma_cb, max(cbcd.id),cd.ten_cd, gt.ten_gt, gt.thoigian_nhan, cd.dinhmuc, lgt.ten_loaigt ,lgt.heso_loaigt,khoa.ten_khoa, cbgt.ten_loaivt, gt.sluong_thamgia  FROM `GiaiThuong` gt INNER JOIN `CanBo_GiaiThuong` cbgt on gt.ma_gt=cbgt.ma_gt INNER JOIN `Canbo` cb on cb.ma_cb=cbgt.ma_cb INNER JOIN `CanBo_ChucDanh` cbcd on cbcd.ma_cb=cb.ma_cb INNER JOIN `NamHoc` nh on gt.ma_nh = nh.ma_nh INNER JOIN `ChucDanh` cd on cd.ma_cd=cbcd.ma_cd INNER JOIN `LoaiGiaiThuong` lgt ON lgt.ma_loaigt=gt.ma_loaigt INNER JOIN `Khoa_PB` khoa on khoa.ma_khoa = cb.ma_khoa WHERE nh.ma_nh = $a and cb.user_id = $user_id and gt.trangthai = 1 GROUP by gt.ma_gt,cb.ma_cb";
...
$re3 = $conn->query($sql3);
    ...
    echo "</tbody>";
	// var_dump(json_encode($listtonghop,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES));
