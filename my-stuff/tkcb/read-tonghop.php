...
include '../../nhiemvu.php';
	...
	$sql5= "SELECT ID FROM `realdev_users` WHERE `user_email` = '$b' ";
	$re5 = $conn->query($sql5);
	error_log('sql = ' . $sql5);
		$row5 = $re5->fetch_assoc();
	$user_id = $row5['ID'];


	$sql = "SELECT cb.ma_cb, cdt.giochuan, dtcb.ten_loaivt,dtnc.sluong_thamgia, k.ten_khoa FROM `DeTai_NCKH` dtnc INNER JOIN `DeTai_CanBo` dtcb on dtnc.ma_dtnckh=dtcb.ma_dtnckh INNER JOIN `Canbo` cb on cb.ma_cb=dtcb.macb INNER JOIN `NamHoc` nh on dtnc.ma_nh = nh.ma_nh INNER JOIN `CapDeTai` cdt ON cdt.ma_cdt=dtnc.ma_cdt INNER JOIN `Khoa_PB` k on k.ma_khoa=cb.ma_khoa WHERE nh.ma_nh =$a and cb.user_id = $user_id and dtnc.trangthai = 1;";
	...
	$sql2 = "SELECT gt.ma_gt,cb.ma_cb, lgt.heso_loaigt, lgt.ten_loaigt, k.ten_khoa,  gt.sluong_thamgia  FROM `GiaiThuong` gt INNER JOIN `CanBo_GiaiThuong` cbgt on gt.ma_gt=cbgt.ma_gt INNER JOIN `Canbo` cb on cb.ma_cb=cbgt.ma_cb INNER JOIN `NamHoc` nh on gt.ma_nh = nh.ma_nh INNER JOIN `LoaiGiaiThuong` lgt ON lgt.ma_loaigt=gt.ma_loaigt INNER JOIN `Khoa_PB` k on k.ma_khoa=cb.ma_khoa WHERE nh.ma_nh =$a and cb.user_id = $user_id and gt.trangthai = 1";
...
while($row22 = $re22->fetch_assoc()){
	...
	while ($row = $re2->fetch_assoc()) {
		...
		foreach ($listtonghop as $item) {
			if($row['ma_cb']==$item->getMacb()){
				if (str_contains($row['ten_loaigt'], 'cấp khoa')) {
					...
					$item->setHuongsv(number_format($temp+($row['heso_loaigt']/$row['sluong_thamgia']),2));
				}
				else if(str_contains($row['ten_loaigt'], 'Olympic')) {
					$temp = $item->getOlympic();
					$item->setOlympic(number_format($temp+($row['heso_loaigt']/$row['sluong_thamgia']),2));
				}
				else{
					
					$temp = $item->getSvnckh();
					$item->setSvnckh(number_format($temp+($row['heso_loaigt']/$row['sluong_thamgia']),2));
				}
				$flag=true;
				break;
		...
		if(!$flag){
			...
			$c->setNamhoc($namhoc);
			if (str_contains($row['ten_loaigt'], 'cấp khoa')) {
				$c->setHuongsv(number_format($row['heso_loaigt']/$row['sluong_thamgia'],2));
			}
			else if(str_contains($row['ten_loaigt'], 'Olympic')) {
				$c->setOlympic(number_format($row['heso_loaigt']/$row['sluong_thamgia'],2));
				
			}
			else{
				$c->setSvnckh(number_format($row['heso_loaigt']/$row['sluong_thamgia'],2));
	...
	$sql3 = "SELECT cb.ma_cb, lsl.ten_loaisl, lsl.giatri_sl, k.ten_khoa,lctk.ten_loai,cbct.ten_loaivt,ctk.sluong_thamgia,sotinchi FROM `CongTrinh_Khac` ctk INNER JOIN `CanBo_Ctr` cbct on ctk.ma_ctr=cbct.ma_ctr INNER JOIN `Canbo` cb on cb.ma_cb=cbct.ma_cb INNER JOIN `NamHoc` nh on ctk.ma_nh = nh.ma_nh INNER JOIN `LoaiSL_TC` lsl ON lsl.ma_loaisl=ctk.ma_loaisltc INNER JOIN `LoaiCongTrinh_Khac` lctk ON lctk.ma_loai=lsl.ma_loaict INNER JOIN `Khoa_PB` k on k.ma_khoa=cb.ma_khoa WHERE nh.ma_nh =$a and cb.user_id = $user_id and ctk.trangthai = 1";
	...
	echo "</thead>";
		$sqlextra = "SELECT max(cd.dinhmuc) as dinhmucmax FROM `CanBo_ChucDanh` as cbcd inner join `ChucDanh` as cd on cbcd.ma_cd = cd.ma_cd inner join `Canbo` as cb on cbcd.ma_cb=cb.ma_cb INNER JOIN `realdev_users` as u on u.ID=cb.user_id WHERE cb.user_id=".$user_id;
		...
		if($ree2!=false && $ree2->num_rows>0){
			...
			while($row2 = $ree2->fetch_assoc()){
				...
				$dinhmuctam = $row['dinhmucmax'] * ($row2['mucgiam']*$tilesothang);
				$dinhmuc_duocgiam+=$dinhmuctam;
				if($i==0)
				$note .= $row2['ten_gtr'];
				else
				$note .= ','.$row2['ten_gtr'];
			...
			if($row['dinhmucmax']-$dinhmuc_duocgiam>=0){
				$dinhmucnew = $row['dinhmucmax']-$dinhmuc_duocgiam;
			}else{
				$dinhmucnew = 0;
			}
