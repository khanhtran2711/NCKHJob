...
$sql4= "SELECT namhoc FROM `NamHoc` WHERE ma_nh = $a ";
    ...
    $sql1 = "SELECT gt.ma_gt,cb.ma_cb, cb.user_id, max(cbcd.id),cd.ten_cd, gt.ten_gt, gt.thoigian_nhan, cd.dinhmuc, lgt.ten_loaigt ,lgt.heso_loaigt,khoa.ten_khoa,cbgt.ten_loaivt,gt.sluong_thamgia FROM `GiaiThuong` gt INNER JOIN `CanBo_GiaiThuong` cbgt on gt.ma_gt=cbgt.ma_gt INNER JOIN `Canbo` cb on cb.ma_cb=cbgt.ma_cb INNER JOIN `CanBo_ChucDanh` cbcd on cbcd.ma_cb=cb.ma_cb INNER JOIN `NamHoc` nh on gt.ma_nh = nh.ma_nh INNER JOIN `ChucDanh` cd on cd.ma_cd=cbcd.ma_cd INNER JOIN `LoaiGiaiThuong` lgt ON lgt.ma_loaigt=gt.ma_loaigt INNER JOIN `Khoa_PB` khoa on khoa.ma_khoa = cb.ma_khoa WHERE nh.ma_nh = $a and cb.ma_khoa=$b and gt.trangthai = 1 GROUP by gt.ma_gt,cb.ma_cb, cb.user_id;";
