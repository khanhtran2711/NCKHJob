...
include '../../nhiemvu.php';
	...
	$sql2 = "SELECT cb.user_id,cb.ma_cb, lgt.heso_loaigt, lgt.ten_loaigt FROM `GiaiThuong` gt INNER JOIN `CanBo_GiaiThuong` cbgt on gt.ma_gt=cbgt.ma_gt INNER JOIN `Canbo` cb on cb.ma_cb=cbgt.ma_cb INNER JOIN `NamHoc` nh on gt.ma_nh = nh.ma_nh INNER JOIN `LoaiGiaiThuong` lgt ON lgt.ma_loaigt=gt.ma_loaigt WHERE nh.ma_nh =$a and cb.ma_khoa=$b and gt.trangthai = 1 GROUP BY cb.ma_cb;";
	...
	$sql3 = "SELECT cb.user_id,cb.ma_cb, lsl.ten_loaisl, lsl.giatri_sl, lctk.ten_loai,cbct.ten_loaivt,ctk.sluong_thamgia FROM `CongTrinh_Khac` ctk INNER JOIN `CanBo_Ctr` cbct on ctk.ma_ctr=cbct.ma_ctr INNER JOIN `Canbo` cb on cb.ma_cb=cbct.ma_cb INNER JOIN `NamHoc` nh on ctk.ma_nh = nh.ma_nh INNER JOIN `LoaiSL_TC` lsl ON lsl.ma_loaisl=ctk.ma_loaisltc INNER JOIN `LoaiCongTrinh_Khac` lctk ON lctk.ma_loai=lsl.ma_loaict WHERE nh.ma_nh =$a and cb.ma_khoa=$b and ctk.trangthai = 1 GROUP BY cb.ma_cb;";
